...
def products():
    return [
        Product(
            ...
            instock=True, 
            name="Планшет Apple iPad Pro Wi-Fi (2020), 128 GB, серый", 
            price=69990.0
        ),
        Product(
        ...
        Product(
            ...
            instock=True,
            name="Кольцо JV Джей Ви из белого золота 585 пробы с бриллиантами",
            price=63041.0
        ),
        Product(
...
def test_ozon_scraper(URLs, products):
    ...
    for i, url in enumerate(URLs):
        ...
        assert scraped_product.image.startswith("https://cdn1.ozone.ru")
        assert (products[i].price - 10000) <= scraped_product.price <= (products[i].price + 10000)
